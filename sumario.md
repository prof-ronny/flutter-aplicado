---
title: Sumário – Flutter Aplicado
description: Sumário oficial do livro Flutter Aplicado, com capítulos e subcapítulos conforme a estrutura original da obra.
lang: pt-BR
---

# Sumário do livro Flutter Aplicado

## Prefácio

---

## Capítulo 1 – Introdução ao Flutter

- 1.1 História e Desenvolvimento do Flutter  
- 1.2 Vantagens e Uso no Desenvolvimento  
- 1.3 Configuração do Ambiente de Desenvolvimento  
- 1.4 Hello World: Seu Primeiro Aplicativo Flutter  
- 1.5 Resumo do Capítulo  

---

## Capítulo 2 – Fundamentos do Dart

- 2.1 Sintaxe Básica de Dart  
- 2.2 Funções  
- 2.3 Escopo em Dart  
- 2.4 Projeto: Calculadora Simples em Dart  
- 2.5 Classes e recursos avançados de Dart  
- 2.6 Resumo do Capítulo  

---

## Capítulo 3 – Flutter SDK

- 3.1 O ecossistema do Flutter SDK e suas ferramentas  
- 3.2 Criando um Projeto  
- 3.3 Estrutura nativa Android  
- 3.4 Estrutura nativa para iOS  
- 3.5 Versão Web do Flutter  
- 3.6 Pastas Desktop – Windows, macOS e Linux  
- 3.7 Resumo do Capítulo  

---

## Capítulo 4 – Widgets Básicos em Flutter

- 4.1 Entendendo o que são Widgets  
- 4.2 MaterialApp, Scaffold e AppBar  
- 4.3 Text Widget em Flutter  
- 4.4 Row e Column  
- 4.5 Container  
- 4.6 Projeto: App de Perfil Pessoal  
- 4.7 Resumo do Capítulo  

---

## Capítulo 5 – Temas e Design System (Material 3)

- 5.1 O que é Design System?  
- 5.2 Ativando Material 3 no Flutter  
- 5.3 ColorScheme: o coração das cores no Material 3  
- 5.4 Typography no Material 3  
- 5.5 ShapeTokens em Material 3  
- 5.6 Tokens de Espaçamento no Material 3  
- 5.7 Componentes Material 3  
- 5.8 Tokens no Material 3 (Design Tokens)  
- 5.9 Projeto prático – Construindo um app Flutter com Material 3  
- 5.10 Resumo do Capítulo  

---

## Capítulo 6 – StatelessWidget e StatefulWidget

- 6.1 Conceito de Widgets com e sem Estado  
- 6.2 Ciclo de Vida dos Widgets  
- 6.3 Organização de Código  
- 6.4 Resumo do Capítulo  

---

## Capítulo 7 – Botões e Interações no Material 3

- 7.1 Tipos de Botões  
- 7.2 Estados e Estilos  
- 7.3 Boas Práticas de Interação  
- 7.4 Resumo do Capítulo  

---

## Capítulo 8 – Widgets de Mídia e Ícones

- 8.1 Trabalhando com Imagens  
- 8.2 Organização de Assets  
- 8.3 Ícones no Flutter  
- 8.4 Resumo do Capítulo  

---

## Capítulo 9 – Listas e Rolagem de Conteúdo

- 9.1 ListView  
- 9.2 ListView.builder  
- 9.3 Organização de Listas  
- 9.4 Projeto prático – Lista de Contatos  
- 9.5 Resumo do Capítulo  

---

## Capítulo 10 – Formulários e Entrada de Dados

- 10.1 TextField e TextFormField  
- 10.2 Validação de Formulários  
- 10.3 Projeto prático – Formulário de Cadastro  
- 10.4 Resumo do Capítulo  

---

## Capítulo 11 – Navegação no Flutter

- 11.1 Navegação com Navigator  
- 11.2 Rotas Nomeadas  
- 11.3 Projeto prático – Navegação com Navigator  
- 11.4 Projeto prático – Mini app multi-páginas com go_router  
- 11.5 Resumo do Capítulo  

---

## Capítulo 12 – Gerência de Estado

- 12.1 Conceito de Estado  
- 12.2 setState  
- 12.3 Introdução ao Provider  
- 12.4 Projeto prático – Lista de Tarefas com Provider  
- 12.5 Resumo do Capítulo  

---

## Capítulo 13 – Consumo de APIs REST

- 13.1 Conceitos de API  
- 13.2 Requisições HTTP  
- 13.3 Tratamento de Dados JSON  
- 13.4 Projeto prático – App com API Real  
- 13.5 Resumo do Capítulo  

---

## Capítulo 14 – Persistência de Dados e Cache

- 14.1 Persistência Local  
- 14.2 Integração com API  
- 14.3 Projeto prático – Lista de Produtos com Cache Local  
- 14.4 Resumo do Capítulo  

---

## Capítulo 15 – Recursos do Dispositivo

- 15.1 Permissões  
- 15.2 Câmera  
- 15.3 Mapas  
- 15.4 Projeto prático – App com Mapa e Câmera  
- 15.5 Resumo do Capítulo  

---

## Capítulo 16 – Autenticação

- 16.1 Fluxo de Login  
- 16.2 Organização de Telas  
- 16.3 Projeto prático – Mini App com Login  
- 16.4 Resumo do Capítulo  

---

## Capítulo 17 – Animações no Flutter

- 17.1 Conceitos de Animação  
- 17.2 Transições e Interações  
- 17.3 Projeto prático – App com Animações  
- 17.4 Resumo do Capítulo  

---

## Capítulo 18 – Layout Responsivo e Adaptativo

- 18.1 Interface Adaptativa  
- 18.2 Navegação Responsiva  
- 18.3 Projeto prático – Interface Adaptativa  
- 18.4 Resumo do Capítulo  

---

## Capítulo 19 – Flutter Web e PWA

- 19.1 Flutter Web  
- 19.2 Progressive Web Apps  
- 19.3 Funcionamento Offline  
- 19.4 Projeto prático – Diário Offline (PWA)  
- 19.5 Resumo do Capítulo  

---

## Capítulo 20 – Flutter Desktop

- 20.1 Introdução ao Flutter Desktop  
- 20.2 Diferenças de Ambiente  
- 20.3 Projeto prático – Gerenciador de Notas Desktop  
- 20.4 Resumo do Capítulo  

---

## Considerações Finais
